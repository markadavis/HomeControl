sap.ui.define([
	...
	"sap/m/MessageBox"
], function(UI5Object, Device, ResourceModel, MessageBox) {
	"use strict";

	return UI5Object.extend("com.davis.homecontrol.lib.ErrorHandler", {
		 ...
		 * @public
		 * @alias com.davis.homecontrol.lib.ErrorHandler
		 */
		constructor: function(oComponent) {
			...
			if (oComponent) {
				...
				this._sComponentId = "com.davis.homecontrol.lib.ErrorHandler";
			}
		...
		hasBusinessException: function(oResponse) {
			...
			for (var i = 0; i < aResponses.length; i++) {
				...
				if (oResp.statusCode && (parseInt(oResp.statusCode, 10) < 200 
											|| parseInt(oResp.statusCode, 10) > 299)) {
					oError = this.parseODataError({
						responseText: oResp.body
					});
					oError.error.title = this._oResourceBundle.getText("BusExceptionTitleError");
		...
		_parseJSON: function(sJson) {
			...
			try {
				...
				jQuery.sap.log.error("JSON parser error", "Unable to parse JSON message.", this._sComponentId);
			}
		...
		_parseXML: function(sXml) {
			...
			try {
				...
				jQuery.sap.log.error("XML parser error", "Unable to parse XML message.", this._sComponentId);
			}
		...
		_parseHTML: function(sHtml) {
			...
			try {
				...
				jQuery.sap.log.error("HTML parser error", "Unable to parse HTML message.", this._sComponentId);
			}
